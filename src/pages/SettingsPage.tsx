        ...
        <IonContent>
          <IonList>
            <IonItem>
              ...
              <IonLabel className='ion-text-wrap uiFont'><a href="https://github.com/MrMYHuang/TwoFauth#report" target="_new">啟用 app 異常記錄</a></IonLabel>
            ...
            <IonItem hidden={!this.props.hasAppLog}>
              ...
              <IonButton fill='outline' shape='round' slot='end' size='large' className='uiFont' onClick={e => {
                window.open(`mailto:myh@live.com?subject=TwoFauth異常記錄回報&body=${encodeURIComponent("問題描述(建議填寫)：\n\n瀏覽器：" + navigator.userAgent + "\n\nApp版本：" + PackageInfos.pwaVersion + "\n\nApp設定：" + JSON.stringify(this.props.settings) + "\n\nLog：\n" + Globals.getLog())}`);
              }}>回報</IonButton>
            </IonItem>
            <IonItem>
              <div tabIndex={0}></div>{/* Workaround for macOS Safari 14 bug. */}
